<template>
  <div class="cus-container">
    ...
    >
      ...
      >
        <template slot-scope="{row,$index}">
          ...
          <el-button size="mini" type="primary" @click="clickFetchBackLink(row, $index)">{{
							$t('table.fetchBackLink')
					}}</el-button>
          <el-button size="mini" type="primary" @click="clickAddressList(row, $index)">{{
							$t('table.mailList')
					}}</el-button>
        </template>
      </el-table-column>
    ...
    <el-dialog :title="$t('table.fetchBackLink')" :visible.sync="fetchbackPaymentLinkModalShow" direction="rtl" width="800px">
			<div style="text-align: center; margin-bottom: 40px;">
				<h2 style="color: rgba(48, 49, 51, 1);font-size: 16px;">{{$t('table.fetchBackLink')}}</h2>
			...
			<div class="modal_foot">
				<div style="text-align: center;">
					<el-button @click="fetchbackPaymentLinkModalShow = false;globPayRepaymentLink = ''" size="mini">{{ $t('tagsView.close') }}</el-button>
				</div>
			</div>
		</el-dialog>
    <BorrowingInfo
		...
		<addressList :list.sync="childAddressList" ref="child" :showDialog='showDialog'/>
  ...
  export default {
    ...
    methods: {
      ...
      fetchData() {
        ...
        CollectionNewList(this.listQuery).then(response => {
