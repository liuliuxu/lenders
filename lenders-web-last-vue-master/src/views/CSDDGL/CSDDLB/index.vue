<template>
	<div class="cus-container">
        ...
        <div class="fenpei" style="font-size: 14px" v-if="multipleSelectionCSRY && multipleSelectionCSRY.length">已选择{{multipleSelectionCSRY.length}}条</div>
		<el-table
                ...
                <template slot-scope="{row,$index}">
                    <div style="margin-bottom: 10px;">
                      ...
                      <el-button size="mini" type="primary"
                                 ...
                                 @click="clickSendSms(row,$index)">{{ $t('table.send') }} <span v-if="row.sendmsgcount">({{row.sendmsgcount}})</span> </el-button>
					</div>
					<div style="margin-bottom: 10px;">
                    	<el-button size="mini" type="primary" @click="clickAddRemarks(row,$index)">{{ $t('table.addRemarks') }}</el-button>
                    	<el-button size="mini" type="primary" @click="clickViewRemarks(row,$index)">{{ $t('table.viewRemarks') }}</el-button>
					</div>
					<div style="margin-bottom: 10px;">
						<el-button size="mini" type="primary" @click="clickFetchBackLink(row, $index)">{{ $t('table.fetchBackLink') }}</el-button>
						<el-button size="mini" type="primary" @click="clickAddressList(row, $index)">{{ $t('table.mailList') }}</el-button>
					</div>
					<div style="margin-bottom: 10px;">
						<el-button size="mini" type="primary" @click="clickRenewLink(row, $index)">{{ $t('table.renewLink') }}</el-button>
					</div>
                </template>
            </el-table-column>
...
<!-- 		      <el-button
		...
		<el-dialog
				...
				:visible.sync="addRemarksShowModal"
				direction="rtl"
				width="800px"
		>
			<div>
          ...
          <el-form-item :label="'Tag'" prop="TagKey">
            <el-radio-group v-model="formRemarks.TagKey">
              <el-radio v-for="(tag, index) in collectionTags" :label="tag.TagKey" style="margin-bottom: 10px;">
		...
		<el-dialog
				:title="$t('table.viewRemarks')"
				:visible.sync="viewRemarksShowModal"
				direction="rtl"
				width="800px"
		>
			<div style="margin-bottom: 20px;">
				<el-table
						v-loading="viewRemarksLoading"
						:data="viewRemarksList"
						element-loading-text="Loading"
						fit
						highlight-current-row
				>
					<el-table-column v-for="(widget, index) in remarksConfig" :label="widget.name" :width="widget.width" align="left">
						<template slot-scope="scope">
							{{ scope.row[widget.key] }}
						</template>
					</el-table-column>
		...
		<el-dialog :title="BackShow? $t('table.fetchBackLink'): $t('table.fetchRenewkLink')" :visible.sync="fetchbackPaymentLinkModalShow" direction="rtl" width="800px">
			<div style="text-align: center; margin-bottom: 40px;">
				<h2 style="color: rgba(48, 49, 51, 1);font-size: 16px;">{{BackShow?$t('table.fetchBackLink'): $t('table.fetchRenewkLink')}}</h2>
				<img style="width: 200px;height: 87px;" src="@/assets/link/link1.png" alt="">
				<h2 style="color: rgba(255, 184, 69, 1);font-size: 14px;">{{ $t('table.link20minuteValid') }}</h2>
				<div>{{ globPayRepaymentLink }}</div>
			</div>
			<div class="modal_foot">
				<div style="text-align: center;">
					<el-button @click="fetchbackPaymentLinkModalShow = false;globPayRepaymentLink = '';BackShow=false" size="mini">{{ $t('tagsView.close') }}</el-button>
				</div>
			</div>
		</el-dialog>
    <BorrowingInfo
		...
		<addressList :list.sync="childAddressList" ref="child" />
...
<script>
	...
	import addressList from '@/components/addressListDialog/index';
	import {
		CollectionList,
		GetCollectionManList,
		AllotCollection,
		SetCollRemark,
		SearchCollRemark,
		CollectionMobileClick,
		SendRepaymentLoans,
		GetGlobPayRepaymentLink,
		GetGlobPayRenewRepaymentLink
	} from '@/api/user';
	...
	const tableConfig=[
        {key:'Remark',name:'备注',width:'120'},
    	{key:'TagName_Cn',name:'标签'},
		{key:'RenewAmount',name:'展期金额', width: '120'},
		{key:'Name',name:'借款人姓名',width:'200'},
		{key:'OrderId',name:'借款单号',width:'220'},
		{key:'Mobile',name:'手机号'},
	...
	const _tableConfig=[
		...
		{key:'RenewAmount',name:'RenewAmount', width: '120'},
		{key:'Name',name:'Name',width:'200'},
		{key:'OrderId',name:'OrderId',width:'220'},
        ...
        {key:'OverDueFine',name:'OverDueFine'},
	...
	const _formConfig=[
		...
		[{key:'OverDueDays',name:'OverDueDays',type:'input',disabled:true},{key:'OverDueFine',name:'OverDueFine',type:'input',disabled:true}],
    ...
    {key:'TagName_En',name:'Tag'},
    {key:'RemarkType',name:'RemarkType'},
		{key:'Remark',name:'Remark',width:'220'},
		{key:'RemardDate',name:'RemarkDate'}
	];
	export default {
		components:{ searchBar,pageForm,Pagination,BorrowingInfo,addressList },
		data() {
			return {
				...
				listCSRY: null,
				orderItem: null,
				globPayRepaymentLink: '', // 还款链接
				fetchbackPaymentLinkModalShow: false, // 获取 获取还款链接弹出框显示状态
        ...
        row.isDisabled = true;
          ...
          row.isDisabled = false;
			 ...
			 * 点击获取还款链接
			 */
			clickFetchBackLink(row, index) {
				this.BackShow = true;
				this.fetchbackPaymentLinkModalShow = true;
				GetGlobPayRepaymentLink({
					"OrderId": row.OrderId,
					"Mobile": row.Mobile
				}).then(res => {
					this.globPayRepaymentLink = res.data;
				});
			},
			// 查看通讯录列表
			clickAddressList(row, index){
				this.$refs.child.clickAddressList(row);
				this.$refs.child.getRow(row);
				// this.childAddressList = row;
			},
			fetchRemarksInfo() {
				this.viewRemarksLoading = true;
				const data = {
					RecordID: this.orderItem.ID,
          			RemarkStatu: this.RemarkStatu
				};
			...
			// 获取展期链接
			clickRenewLink(row, index){
				this.BackShow = false;
				this.fetchbackPaymentLinkModalShow = true;
				console.log(row)
				GetGlobPayRenewRepaymentLink({
					"Mobile": row.Mobile,
					"PaymentAmount": row.RenewAmount,
					"OrderId": row.OrderId
				}).then(res => {
					this.globPayRepaymentLink = res.data;
				});
