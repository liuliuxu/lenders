<template>
	<div class="cus-container">
				...
				<template slot-scope="{row,$index}">
					<div style="margin-bottom: 10px;">
						...
						<el-button size="mini" type="primary" :disabled="row.isDisabled"
							@click="clickSendSms(row, $index)">{{ $t('table.send') }} <span
								v-if="row.sendmsgcount">({{ row.sendmsgcount }})</span> </el-button>
					</div>
					<div style="margin-bottom: 10px;">
		...
		<BorrowingInfo v-if="showDetail" :selectedItem="selectedItem" :showDetail="showDetail"
			...
			<!-- 通讯录列表 -->
		<addressList :list.sync="childAddressList" ref="child" :parentType="'DF1'" />
	</div>
</template>
...
export default {
	...
	methods: {
		...
		fetchData() {
			...
			DF1CollectionList(this.listQuery).then(response => {
