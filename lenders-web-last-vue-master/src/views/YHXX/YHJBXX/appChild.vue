...
<script>
	...
	import { GetAppUpLoadList } from '@/api/user'
	const searchConfig=[
		{key:'Name',name:'姓名',type:'input',value:''},
		{key:'Mobile',name:'手机号',type:'input',value:''},
	]
	const tableConfig=[
		{key:'appName',name:'app名称'},
		{key:'appPackageName',name:'app包名'},
		{key:'appCreteDate',name:'app创建时间'}
	]
	export default {
		props:{
			rowItem:{
		...
		methods: {
		  fetchData() {
			...
			GetAppUpLoadList(this.listQuery).then(response => {
				if(response.error.ErrorCode !== 0){
					this.list = [];
		      		this.total = response.tolal;
		      		this.listLoading = false;
				}else{
					this.list = response.AppAppModelList || response.data;
					this.total = response.tolal;
					this.listLoading = false;
				}
		    })
