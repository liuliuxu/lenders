...
<script>
	...
	const searchConfig=[
		...
		{key:'Name',name:'通讯录名称',type:'input',value:''},
		{key:'phone',name:'通讯录手机号',type:'date',value:''}
	]
	const tableConfig=[
		{key:'name',name:'名称'},
		{key:'phone',name:'手机号'}
	...
	export default {
		...
		methods: {
		  fetchData() {
			...
			GetMailUpLoadList(this.listQuery).then(response => {
		      this.list = response.AppMailModelList || response.data;
